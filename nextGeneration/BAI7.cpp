...
using namespace std;
int X[MAX], n, OK; 
//thiet lap hoan vi cuoi cung X[]=n, n-1,..,1
void Init(void) {
	cin>>n; OK = true;
	//hoan vi cuoi cung X[]=n, n-1,..,1
...
void Permutation(void) {
	Init();//thiet lap hoan vi lon nhat
	while(OK) { //lap khi chua phai hoan vi cuoi cung
		Result(); //dua ra hoan vi hien tai
		Next_Permutation();//sinh ra hoan vi nguoc ke tiep
	}
	cout<<endl;//ket thuc mot test
}
int main(void) {
	int T; cin>>T;//dua vao so luong test T
	while(T--){//lap den test cuoi cung
		Permutation();//dua ra ket qua moi test
	}
}
