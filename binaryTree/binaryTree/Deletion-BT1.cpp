...
void deletion( Node* root, int key) { 
    ...
    if (key_node != NULL) { //neu tim thay key_node
        ...
        //loai node sau nhat temp
		q.push(root);   //dua node goc vao queue
    	// duet theo muc cho den node cuoi cung
    	Node* d_node; //tao node trung gian temp
    	while (!q.empty()) { //lap khi nao hang doi rong
        	d_node = q.front(); //lay node dau hang doi
        	q.pop(); //loai node dau hang doi
        	if ( d_node == temp) { //neu temp la d_node
            	d_node = NULL; //cho temp = NULL
            	delete (temp); //giai phong d_node
            	return ; 
        	} 
        	if (d_node->right) { //neu temp co node con phai
            	if (d_node->right == temp) { //neu node con phai la d_node
                	d_node->right = NULL; //cho no la NULL
                	delete (temp); //giai phong d_node
                	return ; 
            	} 
            	else //neu node con phai khong la d_node
                	q.push(d_node->right); //dua vao hang doi
        	}   
        	if (d_node->left) { //neu temp co node con trai
            	if (d_node->left == temp) { //neu node con trai la d_node
                	d_node->left = NULL; //cho no la NULL
                	delete (temp); //giai phong d_node
                	return ; 
            	} 
            	else //neu node con trai khong la d_node
                	q.push(d_node->left); //dua vao hang doi
        	}	 
    	} 
        //delete(temp);
